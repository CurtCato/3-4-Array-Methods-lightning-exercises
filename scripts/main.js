const businesses = [
    ...
    const outE1 = document.querySelector("#output");
    ...
    agents.forEach(agent => {
      outE1.innerHTML += `<h2>${agent.name.nameFirst} ${agent.name.nameLast}</h2>
      <section>${agent.company}</section>
      <section>${agent.phone}</section>`;
      outE1.innerHTML += "<hr/>"
    })

    //FIND LIGHTNING EXERCISE STARTER
